...
var BytechCoinCrypto
try {
  BytechCoinCrypto = require('bytechcoin-crypto')
} catch (e) {
  /* Silence standardjs check */
  BytechCoinCrypto = e
  BytechCoinCrypto = false
}
...
class CryptoNote {
  ...
  underivePublicKey (derivation, outputIndex, outputKey) {
    ...
    if (BytechCoinCrypto) {
      return BytechCoinCrypto.underivePublicKey(derivation, outputIndex, outputKey)
    } else {
...
function derivePublicKey (derivation, outputIndex, publicKey) {
  ...
  if (BytechCoinCrypto) {
    return BytechCoinCrypto.derivePublicKey(derivation, outputIndex, publicKey)
  } else {
...
function deriveSecretKey (derivation, outputIndex, privateKey) {
  ...
  if (BytechCoinCrypto) {
    return BytechCoinCrypto.deriveSecretKey(derivation, outputIndex, privateKey)
  } else {
...
function generateKeyImage (publicKey, privateKey) {
  ...
  if (BytechCoinCrypto) {
    return BytechCoinCrypto.generateKeyImage(publicKey, privateKey)
  } else {
    const RingSigs = require('./lib/ringsigs.js')
...
function privateKeyToPublicKey (privateKey) {
  ...
  if (BytechCoinCrypto) {
    return BytechCoinCrypto.secretKeyToPublicKey(privateKey)
  } else {
...
function cnFastHash (input) {
  ...
  if (BytechCoinCrypto) {
    return BytechCoinCrypto.cnFastHash(input)
  } else {
...
function generateRingSignature (transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex) {
  ...
  if (BytechCoinCrypto) {
    return BytechCoinCrypto.generateRingSignatures(transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex)
  } else {
    const RingSigs = require('./lib/ringsigs.js')
...
function generateKeyDerivation (transactionPublicKey, privateViewKey) {
  ...
  if (BytechCoinCrypto) {
    return BytechCoinCrypto.generateKeyDerivation(privateViewKey, transactionPublicKey)
  } else {
